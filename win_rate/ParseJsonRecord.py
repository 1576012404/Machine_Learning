...
def parse(source_dir,target_dir,sFileName):
    ...
    source_file=os.path.join(source_dir,sFileName)

    with open(source_file) as jsonfile:
        iTimeMax=-1
        iIndex=0
        iCount = 0
        for row in jsonfile:
            ...
            # elif row_info["type"]=="epilogue":
            #     s=json.loads(row_info["key"])
            #     winner=s["gameInfo_"]["dota_"]["gameWinner_"]
            #     print("winner",winner)

    # print("item_dct.keys",heros_item_dct.keys())
    target_file = os.path.join(target_dir, sFileName)
    df.to_csv("%s.csv"%(target_file))


if __name__=="__main__":
    source_dir="test_data/out_json"##"out_json"
    target_dir="test_data/out_csv"##"out_csv"
    root = Path(source_dir)
    iIndex=0
    for file_name in root.glob("*.json"):
        sName=file_name.name
        csv_name=sName+".csv"
        target_name=os.path.join(target_dir,csv_name)
        if not os.path.exists(target_name):
            parse(source_dir, target_dir, sName)
        if iIndex % 10 == 0:
            print("index", iIndex, sName)
        iIndex+=1
